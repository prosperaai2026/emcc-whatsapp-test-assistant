...
const handleMainMenu = async (remoteJid, text, pushName) => {
...
const handleUnknownIntent = async (remoteJid, text, pushName) => {
    try {
...
const showAgenda = async (remoteJid) => {
    try {
...
const showChurchInfo = async (remoteJid) => {
    try {
        const response = await axios.get(`${BACKEND_API_URL}/info`);
        if (response.data.success) {
...
const handlePrayerRequest = async (remoteJid, text) => {
    try {
...
const handleVisitorName = async (remoteJid, text) => {
    if (sessions[remoteJid]) {
...
const handleVisitorCity = async (remoteJid, text) => {
    if (sessions[remoteJid]) {
